...
import { BufferEncoding } from 'declaration';
 ...
 */
export function getEnumerationValues<E>(enumeration: E): Array<string> {
  return Object.values(enumeration).map((e, index, array) => {
    const result = `${e},`;
 ...
 * Generate random number in a given range
 */
export function generateUid(size = 16, encoding: BufferEncoding = 'hex'): string {
